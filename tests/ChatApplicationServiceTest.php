...
final class ChatApplicationServiceTest extends \PHPUnit\Framework\TestCase // クラス宣言を更新
{
    ...
    public function test_APIキーがない場合にWeb検索エラーメッセージを含めて回答を取得する(): void
    {
        // OpenAI APIキーを空にするために、webSearchToolをnullにしてChatApplicationServiceを再作成
        $chatServiceWithoutKey = new ChatApplicationService(
            self::TARGET_ID_AUTOTEST,
            $this->botRepositoryMock,
            $this->conversationRepositoryMock,
            $this->chatPromptService,
        ...
        // openaiApiKey を空にする
        $reflection = new \ReflectionClass($chatServiceWithoutKey);
        $property = $reflection->getProperty('openaiApiKey');
        $property->setAccessible(true);
        $property->setValue($chatServiceWithoutKey, null);
        ...
        $this->gptMock->expects($this->exactly(2))
            ...
            ->willReturnCallback(function($context, $message) {
                ...
                $this->assertStringContainsString('Error: Web search is not available', $context);
                return 'エラーを考慮した回答';
            });

        $answer = $chatServiceWithoutKey->getAnswer(false, '検索が必要な質問');
