...
final class TimerTriggerTest extends TestCase
{
    ...
    public function test_shouldRunNowが明後日を正しく処理する(): void
    {
        $timeZone = new \DateTimeZone(Consts::TIMEZONE);

        // 現在の「日」を2023-07-01に設定。したがって、トリガーロジックの「明後日」は2023-07-03になります。
        Carbon::setTestNow(Carbon::parse("2023-07-01 10:00:00", $timeZone));
        $trigger = new TimerTrigger("day after tomorrow", "11:00", "テスト");

        // 実際の「明後日」(2023-07-03)のテスト時刻
        $testTimeOnActualDayAfterTomorrow = Carbon::parse("2023-07-03 11:00:00", $timeZone);
        Carbon::withTestNow($testTimeOnActualDayAfterTomorrow, function() use ($trigger) {
            $this->assertTrue($trigger->shouldRunNow(5), "実際の「明後日」に設定された時刻に実行されるべきです");
        });

        // 「明日」(2023-07-02)のテスト時刻 (実行されないべき)
        Carbon::setTestNow(Carbon::parse("2023-07-02 11:00:00", $timeZone));
